...
The full diff kind list (`shift_initiative_target`, `add_availability`, `set_initiative_status`, `add_dependency`, `remove_dependency`, `reorder_initiatives`, `update_status_check`, `create_child_initiative`) is documented in MC's `pm-soul.md` reference and in the spec at `docs/reference/roadmap-and-pm-spec.md` §9.3 / §9.5.
...
The same principle applies to non-PM tool families that share a scope and authz model: prefer one tool with an `action` enum (see `update_subtask`, `update_note`) over three siblings. Only split when the per-action shapes diverge enough that a unified schema becomes unreadable. See `docs/archive/mcp-surface-review.md` for the full reasoning and the action queue that landed this principle.
