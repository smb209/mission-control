...
Decompose-flow proposals MUST emit at least one `create_convoy_under_initiative` diff and MUST NOT emit `create_task_under_initiative` — the schema in [`src/lib/db/pm-proposals.ts`](../../src/lib/db/pm-proposals.ts) `validateProposedChanges` rejects mismatches at intake. The mandate is unconditional (the `MC_PM_CONVOY_MANDATE` env flag was removed after stabilization). On accept, the apply pass in [`src/lib/convoy-dag.ts`](../../src/lib/convoy-dag.ts) auto-creates the parent task (`status=convoy_active`, mirroring `promote_initiative_to_task`), the `convoys` row carrying `acceptance_criteria` (parent-level operator-facing criteria), and per-slice `convoy_subtasks` in topological order, then fires root slices.
